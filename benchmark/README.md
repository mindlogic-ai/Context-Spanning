...
| role | what | pinned in |
| --- | --- | --- |
| speech model | the released checkpoint, fine-tuned from `nvidia/personaplex-7b-v1` | `mindlogicinc/context-spanning-7b`, `--checkpoint` |
...
first scored item. `run_full.sh` encodes the layout used for the released results (4 x 96 GB):
...
| speech model | Moshi (MoshiRAG fine-tune), one voice | the released checkpoint (PersonaPlex fine-tune), released voices |
