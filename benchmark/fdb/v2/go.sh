...
# benchmark's orchestrator; our side is benchmark/fdb/v2/ours_adapter.js (WebRTC 48 kHz PCM16 <-> our
# WebSocket, raw float32 24 kHz, one 80 ms frame per tick so the frame clock never stops).
#   FDB_V2_DIR=<clone>/v2  OURS_WS_URL=ws://localhost:8080/ws  OPENAI_API_KEY=...  bash benchmark/fdb/v2/go.sh
# The whole staged set (200 tasks), the benchmark's slow examiner and 120 s per task: each task holds a paid
# GPT Realtime session, so a run is hours of audio.
set -euo pipefail
: "${FDB_V2_DIR:?set FDB_V2_DIR}"; : "${OPENAI_API_KEY:?set OPENAI_API_KEY}"
DURATION=120; EXAMINER_MODE=slow
...
echo "[v2] scoring (benchmark's own): bash benchmark/prepare_evaluation.sh $OUT ; bash benchmark/run_evaluation.sh --root_dir $OUT ;"
