"""Context Spanning - single entry point.
  ...
  python main.py infer   --input-wav q.wav --output-wav out.wav [--voice main]
  python main.py serve   [--voice main] [--host 0.0.0.0 --port 8080 --token ...]
...
(MCP_ROUTER_LLM_URL / MCP_ROUTER_LLM_MODEL), the LLM that rewrites agent-addressed questions
...
def _backend_and_asr():
    """The DuetaSpan runtime backend (the MCP tool router over the tool bank: tool call, direct answer
    or nothing) and its ASR client, prewarmed: the first `<ret>` otherwise pays the MCP session set-up,
    the tool discovery and the router's first call."""
    ...
    # would otherwise degrade silently into "every live-value request gets no span".
...
def main(argv=None):
    ...
    p.add_argument("--voice", default="main", help="voice: main (default), f0-f3, m0-m3 from the weights repo, or a local .pt of agent-voice Mimi codes")
    ...
    p.add_argument("--voice", default="main")
    p.add_argument("--text-prompt", default=DEFAULT_PERSONA)
