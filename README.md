...
select GPUs; `ROUTER_MEM` (0.85) sizes the router's share of its GPU — the A4B weights alone are 48.5 GiB and the
8192-token KV cache needs the rest, so the router gets a GPU of its own and the ASR and the speech model share another. The router server also answers the RAG fallback and is the
