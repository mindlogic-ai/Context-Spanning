...
# The DuetaSpan router, direct-answer path: the system prompt the training spans were produced under
# (verbatim), the Context DB framing and the JSON reply. The DuetaSpan tool bank (124 live tools) is
# not part of this package, so the catalog is empty: the LLM answers from knowledge or abstains.
...
class LLMReferenceBackend:
    ...
    def retrieve(self, question: str, context: dict | None = None) -> str:
        ...
        try:
            ...
            return text if len(text) > 2 else NO_INFO
        if isinstance(picked, dict) and picked.get("answer"):
            return str(picked["answer"]).strip()
        return NO_INFO                              # a tool call: no tool bank in this package
