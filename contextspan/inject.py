"""Context Span injection: the router's sentence is read into the running stream as one masked block.
...
from . import spans as seq

__all__ = ["span_ids", "prefill", "pending_exit_cb0"]


def span_ids(reference_text: str, spm) -> list[int]:
    """ch0 ids of the block for the reference exactly as the router returned it."""
    return seq.context_span_ids((reference_text or "").strip(), spm)


def pending_exit_cb0(lm_gen) -> tp.Optional[torch.Tensor]:
...
def prefill(lm_gen, ids: tp.Sequence[int], agent_sil: torch.Tensor,
            user_sine: torch.Tensor) -> int:
    ...
    Columns here are already delayed coordinates: the block is a
    constant column repeated `n` times, differing only in the text row. The model input for block
    column j is column j-1, so the forward is fed [last live column] + block[:n-1].
    """
