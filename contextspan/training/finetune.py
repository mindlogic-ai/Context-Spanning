"""Fine-tuning on prepared tensors: Context Span blocks spliced at sampled delays.
...
def loss_fn(out, codes, tmask, amask, lm):
...
def train(data_dir, out_dir, checkpoint=None, steps=1000, lr=2e-6, accum=8, context=3000,
          ckpt_every=250, seed=0, device="cuda"):
    ...
    for step in range(1, steps + 1):
        ...
        for _ in range(accum):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                loss, tl, al = loss_fn(out, codes, tmask, amask, lm)
            (loss / accum).backward()
            tl_acc += tl / accum
            al_acc += al / accum
