...
def instant_answer(query: str) -> str:
    ...
    it is only tangentially related (a query for "Seoul movies" -> an actor's biography), which is worse
...
def web_search(query: str) -> str:
    ...
    # back the answer the other one already has until its own timeout, which can push a ready
    # Wikipedia sentence past the span deadline so the turn ends as "(no information found)".
    # DuckDuckGo's instant answer is preferred when both arrive together.
