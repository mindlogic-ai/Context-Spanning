...
The remote free sources (Nominatim with its mandatory gap between requests, Overpass, Photon) cannot
answer within a second, and no amount of caching fixes the *first* call; `directions` pays the
Nominatim gap twice, once per endpoint.
...
whole-country extract is far slower to fetch; `_extract` prefers it when it is on disk):
  ...
  GeoNames KR.txt   144k Korean places, nationwide — Seolleung Station, Gangnam-gu,
                    Seongsu-dong, Gyeongju, Busan.
  ...
  BBBike Seoul.pbf  Seoul's OSM extract — every POI kind the tool bank asks for, plus the
                    stations and parks GeoNames omits (Seonjeongneung Station, Banpo Hangang Park).

Geocoding is therefore nationwide; POI search is Seoul-only. `covers()` reports the
bounding box of the POI table, and outside it the adapters must still ask
Overpass — there is simply no local data for Busan's cafes, and a slow answer beats none.
...
# GeoNames feature codes worth geocoding, best first. A question about "Busan" means the
...
def _load_geonames(conn: sqlite3.Connection) -> int:
    ...
    with open(os.path.join(GEO_DIR, "KR.txt"), encoding="utf-8") as handle:
        for line in handle:
            ...
            # Busan Metropolitan City outranks the hamlet that shares its name.
...
def _load_pbf(conn: sqlite3.Connection) -> tuple[int, int]:
    ...
    class Collector(osmium.SimpleHandler):
        ...
        def way(self, way):
            ...
            # nodes alone would answer "parks near Seongsu-dong" with silence.
...
def place(name: str) -> dict[str, Any] | None:
    ...
    with _lock:
        row = conn.execute(
            ...
            (target,)).fetchone()
        if row is None:                       # Seongsu-dong is filed as Seongsu-dong 1-ga / 2-ga
            row = conn.execute(
        ...
        if row is None and target.endswith("역") and len(target) > 2:
            # OSM names Seoul's stations without the "-yeok" (station) suffix — Seonjeongneung,
            # Hansung University — while every Korean says Seonjeongneung-yeok. Drop the
            # suffix and look only among stations, so Gangnam-yeok (Gangnam Station) cannot
            # land on the neighbourhood called Gangnam.
            row = conn.execute(
...
if __name__ == "__main__":
    ...
    # OSM drops the "-yeok" (station) suffix; a Korean speaker never does.
    ...
    # Gyeongju is the whole point of the country-wide extract: on the Seoul-only file the
