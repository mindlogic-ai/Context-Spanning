...
# with it, bare administrative-dong names ("Seongsu-dong", a Seoul neighbourhood) resolve
# too. Free tier, 100k calls/day. Two endpoints: address.json geocodes lot-number/road-name
# addresses ("Yeoksam-dong 640-1", "Teheran-ro 152"), keyword.json resolves place names and
# POIs ("Seongsu-dong", "N Seoul Tower", "Starbucks at Gangnam Station").
...
# the first entry whose word appears in the query wins, so put "hansik" (Korean food)
# before the generic "sikdang" (restaurant) or every Korean-restaurant query collapses
# to plain restaurants.
...
def _clauses(keyword: str) -> list[str]:
    ...
    people actually ask ("find a gas station nearby", asked in Korean).
    """
...
def _kakao(query: str, x: str = "", y: str = "", radius: int = 0) -> list[dict]:
    ...
    OSM has no node called "Seongsu-dong" — Seoul's neighbourhood is filed as
    Seongsu-dong 1-ga / 2-ga, so Nominatim resolves the bare name to a road in Jeju. Kakao carries
...
def _kakao_nearby(centre: str, keyword: str, radius: int) -> tuple[float, float, list]:
    # Anchor via the address-first geocoder, NOT a bare keyword search. `_kakao("Ilsan")`
    # keyword-matches a *business literally named Ilsan* 335 km away, so a radius search around
    # it found no Costco and the tool spent 13 s in Overpass returning nothing. `_kakao_geocode`
    # resolves Ilsan to the district (address.json) first, so the anchor is the real Ilsan.
    ...
    if not hits:
        # A sparse brand ("Costco near Ilsan") sits a few km past a tight radius. Ask Kakao
...
def _kakao_addr(query: str) -> list[dict]:
    ...
    "Yeoksam-dong 640-1" and "Teheran-ro 152" carry a building number; the local OSM gazetteer and
...
# `borough` belongs here: Nominatim files Gangnam-gu as one. Leaving it out told `_places`
# that a perfectly good hit was not place-like, which sent Gangnam-gu down the Overpass
...
# Only an administrative name is worth a second Overpass round-trip: "Seongsu-dong" yes,
# "Seolleung Station" no (Nominatim has the station, it just files it as a highway).
# The regex matches Hangul names ending in an administrative suffix (dong, ga, eup, myeon,
# ri, gu, si, gun).
_ADMIN_NAME = re.compile(r"[가-힣]+(동|가|읍|면|리|구|시|군)$")
# Names Seoul files in numbered pieces — Seongsu-dong is stored as Seongsu-dong 1-ga /
# 2-ga — so an
...
def _named_places(name: str) -> list[dict]:
    ...
    A prefix regex cannot use that index: `["name"~"^Gangnam-gu"]` makes Overpass scan every
    ...
    The regex is still needed (Seoul files Seongsu-dong as Seongsu-dong 1-ga / 2-ga), so it stays as
    a fallback with a server-side timeout that bounds the damage.
    """
    escaped = name.replace('"', '\\"')
    if _SPLIT_NAME.search(name):
        # Seongsu-dong is stored as Seongsu-dong 1-ga / 2-ga, so its exact name matches nothing —
        ...
        # scans; capping it at 10 s made the server abort before it found Seongsu-dong 1-ga and
        # sent Seongsu-dong back to a road in Jeju. Correctness first — the answer is then
...
def _places(address: str, limit: int = 10) -> list[dict]:
    ...
    # that geocodes lot-number/road-name addresses with a building number ("Yeoksam-dong 640-1",
    # "Teheran-ro 152"). It returns None for non-Korean names and on any error, so the keyless
    # path still runs.
    ...
    # twice. It holds Hangul names only, so Seongsu-dong stops here and Times Square does not.
    ...
    # "Seolleung Station" scores 0.000 too, and paying Nominatim's 1 req/s twice for it is waste.
    if best < _IMPORTANCE_FLOOR and not any(_at_home(h) for h in hits[:1]):
        hits += _search(address, limit, biased=True)
    # OSM files Seoul's Seongsu-dong as Seongsu-dong 1-ga / 2-ga, so Nominatim answers the bare name
...
def _rank(hit: dict) -> tuple:
    ...
    Jeju's Seongsu-dong *road* scores importance 5.9e-05 — not fame, just float residue. Sorting
    on it put a country lane ahead of Seoul's Seongsu-dong 1-ga (importance exactly 0.0). Below the
...
def search_places(query: str = "", keywords: str = "", location: str = "",
    ...
    if not centre:                     # "where is N Seoul Tower?" — the word IS the place
        centre, keyword = keyword, ""

    # Kakao is Korea-only. Firing it on a foreign centre finds a spurious Korean namesake —
    # "Nikko" matches a Busan restaurant of that name, so "things to see in Nikko" answered with
    ...
    # reads "Seongsu-dong" as the one in Jeju. Walk the candidates until one actually has the
...
def _render(centre: str, lat: float, lon: float, found: list[tuple]) -> str:
    ...
    No language word joins the two halves: this tool answers "cafes near Seolleung Station"
    asked in Korean and
    ...
    can tell which "Seongsu-dong", or which Springfield, actually answered.
    """
    seen, best = set(), []
...
def transit_directions(origin: str = "", destination: str = "", start: str = "",
    ...
    published GTFS feeds, Seoul's among them, and answers Seolleung Station -> Seonjeongneung
    Station with the bus
...
def distance_matrix(origins: Any = None, destinations: Any = None,
    ...
    # 1x2 matrix from Seoul Station to Daejeon Station and Dongdaegu Station took 10.0 s,
    # while the same two legs
...
def weather(city: str = "", location: str = "", date: str = "", end_date: str = "",
    ...
    "How is the weather in Busan this weekend?" (asked in Korean) carries a date range,
    and dropping the range to answer
