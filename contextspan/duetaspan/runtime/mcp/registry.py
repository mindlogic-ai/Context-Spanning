...
# "this weekend" in "How is the weather in Busan this weekend?" (asked in Korean) and
# answer about right now instead.
_DATE = {"type": "string", "description": "Date in YYYY-MM-DD."}
_END = {"type": "string", "description": "Last date of a range, YYYY-MM-DD."}

# SGD's payment slots describe `amount` as "specified in dollars", so "send 30,000 won"
...
def _world_override() -> dict[str, Callable]:
    ...
    search is worldwide, so "Paris trains" works as well as "Seoul to Busan" (both asked in
    Korean); the intent word is English
