...
class Engine:
    ...
    def __init__(self, checkpoint=None, device="cuda", temp=0.8, temp_text=0.7, cpu_offload=False):
        ...
        with torch.no_grad():
            ...
            # The block forward is a multi-position path the single-step loop never exercises, and its first
            # call in a process is slow; pay it here rather than on the first span of a conversation.
    ...
    def inject_context_span(self, reference: str) -> int:
        ...
        Returns the number of frames the block consumed; `last_prefill_ms` holds its wall-clock cost, which
        grows with the span length: keep references short enough that prefill plus the next step fit the 80 ms
        frame on your GPU."""
