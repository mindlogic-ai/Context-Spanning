...
def test_meta_follows_set_order_and_limit(tmp_path):
    _questions(str(tmp_path))
    meta = B.load_meta(str(tmp_path), sets=("AddSub", "GSM8K"))
    assert [r["id"] for r in meta] == ["AddSub-1", "AddSub-2", "GSM8K-0"]      # AddSub before GSM8K
    assert meta[0] == {"id": "AddSub-1", "dataset": "AddSub", "text": "Joan found 70 seashells .",
                       "answer": "43", "knowledge": None}
    assert [r["id"] for r in B.load_meta(str(tmp_path), sets=("AddSub", "GSM8K"), limit=2)] == ["AddSub-1", "AddSub-2"]
    assert [r["id"] for r in B.load_meta(str(tmp_path), sets=("GSM8K",))] == ["GSM8K-0"]
...
def test_assemble_layout_and_resume(tmp_path):
    out = str(tmp_path)
    _questions(out)
    meta = B.load_meta(out, sets=("AddSub", "GSM8K"))
